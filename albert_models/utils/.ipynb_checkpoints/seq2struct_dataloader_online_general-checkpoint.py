...
class data_generator_single_schema(Dataset):
    def __init__(self, data, tokenizer, max_len, schema, label=0, 
        ...
        self.add_no_overlap = self.task_dict.get('add_no_overlap', True)
            ...
            for role_list in total_target_dict[target_type]:
                ...
                for role_index in role_index_list:
                    ...
                    if argument:
                        ...
                        overlap = set(argument_ids) & set(span_input_ids)
                        ratio = float(len(overlap) / (len(set(argument_ids))+1e-10))
                        if  ratio <= 0.1 and not self.add_no_overlap:
                            print(text, '====', self.tokenizer.decode(span_input_ids), '==span_input_ids==', argument, '=====', self.tokenizer.decode(argument_ids), '==argument_ids==')
                            continue
