...
from nets.spectral_normalization import spectral_norm
from nets.sngp_utils import mean_field_logits
...
"""
https://github.com/kimjeyoung/SNGP-BERT-Pytorch/blob/main/models/sngp.py
...
class SNGP(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids = None,
                attention_mask = None, return_gp_cov = False,
                update_cov = True, forward_mode = "training", mean_field_factor=0.1):

        if forward_mode == "training":
            latent_features = self.backbone(input_ids, token_type_ids, attention_mask)
            latent_feature = latent_features[0]
            ...
            gp_feature, gp_output = self.gp_layer(cls_output, update_cov=update_cov)
            gp_cov_matrix = self.compute_predictive_covariance(gp_feature)
            logit = mean_field_logits(gp_output, gp_cov_matrix, mean_field_factor=mean_field_factor)
            return logit
...
class Deterministic(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids = None,
                attention_mask = None, return_gp_cov = False,
                update_cov = False, forward_mode = "training", mean_field_factor=42):
        if forward_mode == "training":
            latent_features = self.backbone(input_ids, token_type_ids, attention_mask)
            latent_feature = latent_features[0]
